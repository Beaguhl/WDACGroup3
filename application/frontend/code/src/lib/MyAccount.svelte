<script>
	...
	async function getUsername() {
		try {
			const response = await fetch("http://13.53.123.120:8080/my-account", {
				method: "GET",
				headers: {
	...
	async function verifyPasswordForEdit(event) {
		...
		try {
			const response = await fetch("http://13.53.123.120:8080/my-account/verify-password", {
				method: "GET",
				headers: {
					"Content-Type": "application/x-www-form-urlencoded",
					Authorization: "bearer " + $user.accessToken,
	...
	async function updatePassword() {
		try {
			const response = await fetch("http://13.53.123.120:8080/my-account/update-password", {
				method: "PUT",
				headers: {
	...
	async function updateUsername() {
		try {
			const response = await fetch("http://13.53.123.120:8080/my-account/update-username", {
				method: "PATCH",
				headers: {
					"Content-Type": "application/json",
					Authorization: "bearer " + $user.accessToken,
	...
	async function deleteUser() {
		try {
			const response = await fetch("http://13.53.123.120:8080/my-account/delete-account", {
				method: "DELETE",
				headers: {
					"Content-Type": "application/json",
					Authorization: "bearer " + $user.accessToken,
			...
			<div class="container">
				...
				{#if showDeleteAccount}
					<div>
						...
						>
							<button style="background-color: red;" on:click={handleDelete}
								>Yes</button
							>
						</Link>
